...
"""                                     Dev notes
                        DO NOT USE SHELVE, USE PEEWEE OR SOME OTHER DATABASE!!!
    ...
    for nuts in clean_list:
        for numb, x in enumerate(nuts,0):
            ...
            if numb == 3:
                ...
                if count > 1:
                    shop_list.write(str(food) + " x{}\t".format(count) + "\t\t" + str(isle) + "\t\t{0:.2f}".format(float(price)) + "\n")
                else:
                     shop_list.write(str(food) + "\t" + "\t\t" + str(isle) + "\t\t{0:.2f}".format(float(price)) + "\n")
    hidden_fees = 36.00
    tax = (estimate + hidden_fees) * 0.08
    shop_list.write("\n\t\t\t\t\tSubtotal: ${0:.2f}".format(float(estimate + hidden_fees)))
    shop_list.write("\n\t\t\t\t\tTax: ${0:.2f}".format(float(tax)))
    shop_list.write("\n\n\t\t\t\t\tTotal: ${0:.2f}".format(float(estimate + tax + hidden_fees)))
    shop_list.write("\n\t\t\t\t\tplease not that estimate may not be 100% accurate")
    shop_list.write("\n\n" + " - "*25 + "\n")
    clear_screen()
...
def write_files(a_list):
    ...
    shop_list.write('\n\n')
    dinner_list.write(('\t\t') + 'Meal plan for the week of ' + str(datetime.date.today()))
    dinner_list.write('\n\n')
    # write the lists out
...
def dinner_select():
    ...
    clear_screen()
    seven_meals = []  # stores meal names for the week
    counter = 1
    dinner_meal_list.sort(key=lambda dinner_meal_list: dinner_meal_list[0].lower())  # sort by meal name
...
def item_check():
    ...
    for items in forgotten_list:
        ...
        if inny.lower() == "y":
            ...
            continue
    clear_screen()
    print("These items have added these items to the list: ")
    enumer = 1
    for items in add_to_list:
        print(enumer, ") ", items)
        enumer += 1
